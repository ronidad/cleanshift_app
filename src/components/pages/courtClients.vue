...
<button class="btn btn-success" @click="downloadPDF">Download clients for this court </button>
...
export default {
  ...
  methods: {
    downloadPDF(){
      ...
      var colt = ["Name","Phone", "Arrears"];
       
       var rowst = [];
       

        this.courtclients.forEach(client => {      
        var temp = [client.name,  client.phone, client.arrears, ];
        rowst.push(temp);
         }); 
...
//All texts here with cordinates 
        ...
        pdf.save(this.courtName[0].name).pdf;
    },
